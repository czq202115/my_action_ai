...
sudo docker run  -itd --name chat --net $network_name --ip $static_ip2 -v $GITHUB_WORKSPACE/models/:/models/  my-image:latest -m /models/MiniCPM-2B-dpo-fp16-gguf.gguf --port 8000 --host 0.0.0.0 -n 1024
sudo docker ps -a
ip add
